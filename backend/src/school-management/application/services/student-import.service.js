...
const previewImport = async (schoolId, fileBuffer) => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const existingInDb = row.studentId
      ? await findByStudentId(row.studentId)
      : null;
...
const confirmImport = async (schoolId, importToken) => {
  ...
  if (!session) {
    throw new AppError(400, 'Import session not found or expired');
  }
  if (session.schoolId !== schoolId) {
    throw new AppError(403, 'Import token does not match school');
  }
  ...
  const students = session.validRows.map((row) =>
    toBulkStudentData(row, schoolId)
  );
