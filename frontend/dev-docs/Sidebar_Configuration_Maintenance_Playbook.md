...
### Why this approach looks more complex (but scales better)

Direct page-level arrays are simpler for one page, but become expensive to maintain across many pages.

If each page passes custom arrays directly:

- role logic is duplicated in multiple pages
- permission bugs happen when one page is forgotten
- feature updates require editing many files
- different developers implement rules differently

With centralized config + permissions:

- feature structure lives in one feature config file
- role access lives in one permission file
- page code stays thin and consistent
- one rule change updates all pages using the builder

This is why we keep the config system as the default team workflow.
...
Code example is already provided above in:
